...
def extract_number(text):
    ...
    if text.isdigit():
        return int(text)
    return num_words.get(text, None)
...
# -------- [5] 인덱스 → WAV 경로 --------
def get_wav_path_by_index(index, base_dir="/Users/abitria/Downloads/stt"):
    suits = ['spade', 'heart', 'diamond', 'club']
    suit_idx = (index - 1) // 13
    rank_idx = (index - 1) % 13 + 1
    suit = suits[suit_idx]
    filename = f"{rank_idx}s.wav"
    return os.path.join(base_dir, suit, filename)
